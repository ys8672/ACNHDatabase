...
class Rugs extends React.Component {
    ...
    render() {
		...
		let seriesChart = data.reduce(function(obj, v) {
		  var newseries = (v.series).replace(/\s*\(.*?\)\s*/g, '')
		  var cap = newseries.charAt(0).toUpperCase() + newseries.slice(1)
		  obj[cap] = (obj[cap] || 0) + 1;
		  return obj;
		}, {})
		let seriesList = []
		for (const key in seriesChart) {
			let tmp = {label: key, value: seriesChart[key]}
			seriesList.push(tmp)
		}
            ...
            <div>
				...
				<Tabs defaultActiveKey="about" id="uncontrolled-tab-example" mountOnEnter = 'true' class="nav nav-tabs justify-content-center">
				  ...
				  <Tab eventKey="charts" title="Fun Charts">					
					...
					<div class="border border-success">
					  <h3 className='text-center'> Series of Rugs </h3>
					  <div style={{display: 'flex', justifyContent: 'center'}}>
							<BrowserView>
							<BubbleChart 
							graph={{
